...
if ( ! class_exists( 'Wp_License_Manager_Client' ) ) {
    ...
    class Wp_License_Manager_Client {
        ...
        public function add_license_settings_page() {
            $title = sprintf( __( '%s License', $this->text_domain ), 'Kézart' );
        ...
        public function render_licenses_menu() {
            ...
            if ( !$this->is_api_error( $license_info ) ) {
                $msg = __( 'License is valid to <b>%s</b> until <b>%s</b>.', $this->text_domain );
                $msg = sprintf( $msg, $this->product_name, $license_info->license_until );
            ?>
                <div class="updated">
                    <p>
                        <?php echo $msg; ?>
        ...
        public function show_admin_notices() {
            ...
            if ( !$options || ! isset( $options['email'] ) || ! isset( $options['license_key'] ) ||
                ...
                if ( $this->is_api_error( $license_info ) ) {
                    $msg = __( 'Invalid license or license expired to %s.', $this->text_domain );
                    $msg = sprintf( $msg, $this->product_name );
                    ?>
                        <div class="error">
                            <p>
        ...
        public function get_license_acf() {
            $license_info = $this->get_license_info();
            
            if ( $this->is_api_error( $license_info ) ) {
                return false;
            } else {
                return $license_info->license_acf;
            }
        }

        public function get_license_gf() {
            $license_info = $this->get_license_info();
            
            if ( $this->is_api_error( $license_info ) ) {
                return false;
            } else {
                return $license_info->license_gf;
            }
        }
